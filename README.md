...
5. an authenticated UDP join handshake with expiry enforced again at host admission;
...
The current reference suite checks invite round-trip, expiry at both token decoding and host admission, corruption, build mismatch, direct localhost handshake, stable layer result codes, and the game-facing host/join admission flow.
